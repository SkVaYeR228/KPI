...
| **3** Резервування книги | `UC_Reserve (Резервування)` <br> `UC_Login (Авторизація)` <br> `UC_Search (Пошук в каталозі)` | **Пряма реалізація:** `UC_Reserve`. <br> **Залежності:** Вимагає `UC_Login` та `UC_Search` (передумова). |
| **4** Оплата штрафів | `UC_PayFine (Оплата штрафів)` <br> `UC_Login (Авторизація)` | **Пряма реалізація:** `UC_PayFine`. <br> **Залежність:** Вимагає `UC_Login`. |
